<template>
    <el-card shadow="never" class="border-0">
        ...
        <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
            <el-table-column label="会员" min-width="20%">
            ...
            <el-table-column label="会员等级" min-width="20%" align="center">
                <template #default="{ row }">
                    <div v-if="row.last_login_time">
                        <p>最后登录：{{ row.last_login_time }}</p>
                    </div>
                    <div>
            ...
            <el-table-column label="登陆注册" min-width="20%" align="center">
                <template #default="{ row }">
                    {{ row.user_level?.name || "-" }}
                </template>
            </el-table-column>
            ...
            <el-table-column prop="address" min-width="10%" label="操作" align="center">
                <template #default="scope">
                    <small v-if="scope.row.super == 1" class="text-sm text-gray-500">暂无操作</small>
                    <div v-else>
                        <el-button type="primary" size="small" text @click="handleEdit(scope.row)">修改</el-button>
                        <el-popconfirm title="是否要删除该该记录？" confirm-button-text="确认" cancel-button-text="取消"
        ...
        <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
            <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
                ...
                <el-form-item label="头像" prop="avater">
                    <ChooseImage v-model="form.avater" />
                </el-form-item>
